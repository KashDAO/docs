...
| Webhooks               | None                                      | `trade.confirmation-required`, `.completed`, `.failed`, `.rejected` |
...
| ETags / `If-None-Match`| No                                        | Yes — every response, cheap 304 revalidation (no body transfer; still counts toward quota) |
...
The webapp's quote endpoint was the prototype for the `api.kash.bot` one. The public REST API version is now the source of truth. If the two diverge in output (rare; we monitor for divergence), the `api.kash.bot` response is canonical.
