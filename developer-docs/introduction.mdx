...
<Note>
**Kash is non-custodial across every surface.** User funds always sit in ERC-4337 smart accounts controlled by the user's own Privy embedded wallet — Kash never holds keys or funds, on any path. The split below is about **who orchestrates execution**, not custody.
</Note>
...
Use the [Public Embed API](/developer-docs/public-embed-api/overview) to drop iframes and read-only market data into news sites, financial platforms, or community forums. Embeds are read-only — users are directed to app.kash.bot to trade.
