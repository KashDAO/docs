...
<CardGroup cols={2}>
  <Card title="REST API" icon="bolt" href="/developer-docs/rest-api/overview">
    Custodial programmatic access at `api.kash.bot`. Place trades, manage portfolios, receive HMAC-signed webhooks. API-key authenticated. The right surface for bots, agents, and market makers who want Kash to handle wallets, gas, and bridging.
  </Card>
  <Card title="Protocol SDK" icon="cube" href="/developer-docs/protocol-sdk/overview">
    Non-custodial access. Sign transactions directly with your own EOA or smart account against the on-chain AMM. TypeScript and Python implementations with cross-language parity.
  </Card>
...
<Note>
**Which one do I want?**
- Want Kash to manage wallets and bridge funds for you? → **REST API** (custodial).
- Want to sign transactions yourself with your own key? → **Protocol SDK** (non-custodial).
...
### Non-custodial trading & market-making
Use the [Protocol SDK](/developer-docs/protocol-sdk/overview) (TypeScript or [Python](/developer-docs/protocol-sdk-python/overview)) to sign transactions yourself against the on-chain AMM. Combine with the REST API's quote endpoint for live pricing, or quote on-chain directly.
