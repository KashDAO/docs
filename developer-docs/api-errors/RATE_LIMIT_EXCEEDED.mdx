...
For authenticated requests, the limit is **tier-differentiated** — 60 req/min on `free`, 300 req/min on `developer`, custom (admin-set) on `enterprise` and `mm`. See [Rate limits](/developer-docs/rest-api/rate-limits) for the full matrix.
...
- If you consistently bump against the limit during normal operation, upgrade your tier — see [Rate limits](/developer-docs/rest-api/rate-limits) for the upgrade paths. The 60 → 300 req/min jump from `free` → `developer` is 5×, and `enterprise`/`mm` have no application-level cap.
