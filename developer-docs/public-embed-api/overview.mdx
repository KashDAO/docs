...
The Kash Public Embed API enables anyone to embed prediction markets and threads on their website. All embeds are read-only and anonymous — users are directed to app.kash.bot to trade (or to X, via optional tweet deep links).
...
<CardGroup cols={2}>
  ...
  <Card title="Twitter Deep Links" icon="x-twitter">
    Generate Twitter URLs that let users place predictions via tweet (requires the user's opt-in trading bot authorization)
  </Card>
