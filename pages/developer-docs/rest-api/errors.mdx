...
| `401`  | Auth failure (`API_KEY_MISSING`, `API_KEY_INVALID`, `API_KEY_REVOKED`).         |
| `402`  | Spending cap exceeded (rare; typed as `SPEND_LIMIT_EXCEEDED`).                  |
...
| `409`  | Conflict (`IDEMPOTENCY_KEY_CONFLICT`, `TRADE_NOT_AWAITING_CONFIRMATION`, etc.). |
...
| `RATE_LIMITED`               | Per-key quota exceeded. Honour `Retry-After`.                          |
...
| `WEBHOOK_SECRET_NOT_SET`     | Tried to rotate a webhook secret on a key that doesn't have one.       |
| `WEBHOOK_REPLAY_LIMIT`       | More than 5 manual redeliveries for the same event.                    |
| `SIGNATURE_INVALID`          | Opt-in body-signing on `POST /v1/trades` failed verification.          |
